...
pub fn show_detail_panel(app: &mut App, ctx: &egui::Context) {
    ...
    // so we can borrow `pains_state.pains` mutably inside without conflict.
    // Link tuple: (pain_id, segment_id)
                ...
                .show(ui, |ui| {
                    ...
                    ui.vertical(|ui| {
                        ...
                        if !available_segments.is_empty() {
                            ...
                            let combo_key = egui::Id::new("pain_detail_link_seg").with(id);
                            let mut sel: Uuid =
    ...
    if let Some(pair) = link_to_add {
        if !app.customer_page.segment_pain_links.contains(&pair) {
            app.customer_page.segment_pain_links.push(pair);
        }
    }
    if let Some(pair) = link_to_remove {
        app.customer_page.segment_pain_links.retain(|l| l != &pair);
    }
